 ...
 *  - 22/07/2026 (Tinh Leo): Apply to cnma_carshop
 **/
...
def environment = 'CNMA Carshop — Development'
def configApiEndpoint = "${env.DEPLOY_TARGET_CNMA_CARSHOP}"
def deployTargetCfg
def aCustomerTarges
    ...
    try {
        ...
        stage('Prepare Pipeline Data & Config') {
            sendTeamsNotification(teamWebHookEndpoint,
                'Pipeline Started',
                "Pulling latest source and setting up pipeline config from **cnma_carshop**...",
                COLOR_INFO)
            ...
            switch (branch) {
                case 'develop_sprint':
                    environment = 'CNMA Carshop — Development Sprint'
                    aCustomerTarges = deployTargetCfg.develop_sprint
                    break
                case 'qas_internal':
                    environment = 'CNMA Carshop — QAS Internal'
                    aCustomerTarges = deployTargetCfg.qas_internal
                    break
                case 'qas_external':
                    environment = 'CNMA Carshop — QAS External Multi-Customer'
                    aCustomerTarges = deployTargetCfg.qas_external
                    break
                case 'prd_external':
                    environment = 'CNMA Carshop — PRODUCTION Multi-Customer'
        ...
        stage('Build') {
            try {
                echo "Building MTA — cnma_carshop_1.0.0.mtar"
